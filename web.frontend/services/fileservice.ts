...
export async function getTree(
  ...
  const { data } = await axios.get<TreeItemDto[]>("/tree", {
    params: { path, depth },
  });
...
export async function createFile(path: string, content: string = "") {
  const { data } = await axios.post<{ path: string }>("/file", {
    path,
    content,
  });
...
export async function getFileMarkdown(
  path: string,
): Promise<FileMarkdownResponse> {
  const { data } = await axios.get<FileMarkdownResponse>("/v2/preview", {
    params: { path },
  });
...
export async function createFolder(path: string) {
  const { data } = await axios.post<{ ok: boolean; path: string }>("/folder", {
    path,
  });
...
export async function moveFile(moveRequest: MoveRequest) {
  ...
  const { data } = await axios.post("/file/move", moveRequest);
